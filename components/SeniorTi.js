...
export default function JuniorTi(){
  ...
  return(
    <View>
    <Text style={styles.title}> Função de um sênior em programação </Text>
<ScrollView>
    <Image style={styles.img} resizeMode={'contain'} source={require('../assets/junior.png')}/>

    <Text style={styles.paragraph}> No nível sênior, o profissional se depara com uma ampla complexidade de tarefas. É esperado que ele tem maturidade profissional e emocional, poder de decisão e também capacidade para assumir funções de liderança. </Text>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.container}>
              <Text style={styles.items}><AntDesign name="codesquareo" size={24} color="white" /> {item.nome} Sênior</Text>
            </View>)}/>
            </ScrollView>
    </View>
  )
